...
export function Testimonials() {
  return (
    <section className="py-22 bg-brand relative overflow-hidden">
      <div className="absolute top-0 left-1/4 w-96 h-96 rounded-full bg-sky/25 blur-3xl pointer-events-none"></div>
      <div className="absolute bottom-0 right-1/4 w-96 h-96 rounded-full bg-accent/20 blur-3xl pointer-events-none"></div>

      <div className="max-w-6xl mx-auto px-5 lg:px-8 relative z-10">
        <div className="reveal text-center mb-14">
          <span className="section-badge mb-4" style={{ background: 'rgba(255,255,255,0.12)', borderColor: 'rgba(255,255,255,0.2)', color: '#E87A50' }}>
            Témoignages
          </span>
        ...
        <div className="overflow-hidden">
          ...
          >
            {[...TESTIMONIALS, ...TESTIMONIALS].map((t, i) => (
              ...
              >
                <Quote className="w-9 h-9 text-accent-light mb-4 opacity-70" />
                ...
                <div className="flex items-center gap-3 mt-5 pt-5 border-t border-white/10">
                  <img src={t.avatar} alt={t.name} className="w-12 h-12 rounded-full object-cover ring-2 ring-accent-light/40" />
                  ...
                  <div className="flex">
                    {Array.from({ length: t.rating }).map((_, j) => (
                      <Star key={j} className="w-3.5 h-3.5 text-accent-light fill-accent-light" />
                    ))}
                  </div>
                </div>
              </div>
