...
export function ContactPage() {
  ...
  const CONTACT_INFO = [
    ...
    { icon: Phone,   label: 'Téléphone', value: '+237 6 88 15 03 61' },
        ...
        <div className="max-w-7xl mx-auto px-5 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div className="reveal-left">
              ...
              {sent ? (
                <div className="bg-sky-pale border border-sky/30 rounded-3xl p-10 text-center animate-scale-in">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <button
                    ...
                    className="btn-shimmer text-white px-8 py-4 rounded-full font-semibold flex items-center justify-center gap-2 shadow-accent group w-full sm:w-auto disabled:opacity-60 disabled:cursor-not-allowed"
                  >
            ...
            <div className="reveal-right space-y-6">
              <div className="grid sm:grid-cols-2 gap-4">
                {CONTACT_INFO.map((info, i) => {
                  ...
                  return (
                    <div key={i} className="bg-gradient-to-br from-sky-pale to-brand-pale rounded-2xl p-6 card-hover">
              ...
              {/* WhatsApp */}
              <a
                href="https://wa.me/237688150361"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-4 bg-gradient-to-r from-green-500 to-green-600 rounded-2xl p-6 text-white hover:shadow-lg transition-all group"
              >
              ...
              {/* Fictive map */}
              <div className="rounded-3xl overflow-hidden shadow-xl2 h-64 relative bg-sky-pale">
                <div className="absolute inset-0 bg-gradient-to-br from-sky-pale to-brand-pale"></div>
                <svg viewBox="0 0 400 300" className="absolute inset-0 w-full h-full opacity-30">
                  <path d="M0,180 Q100,160 200,170 T400,180 L400,300 L0,300 Z" fill="#3EABD4" />
              ...
              <div className="flex items-center gap-3">
                ...
                <a href={socials.instagram} target="_blank" rel="noopener noreferrer" className="w-11 h-11 rounded-xl bg-slate-100 hover:bg-gradient-to-br hover:from-brand hover:to-sky flex items-center justify-center transition-all hover:scale-110 hover:text-white text-brand">
                  <Instagram className="w-5 h-5" />
                </a>
                <a href={socials.facebook} target="_blank" rel="noopener noreferrer" className="w-11 h-11 rounded-xl bg-slate-100 hover:bg-gradient-to-br hover:from-brand hover:to-sky flex items-center justify-center transition-all hover:scale-110 hover:text-white text-brand">
                  <Facebook className="w-5 h-5" />
                </a>
                <a href={socials.tiktok} target="_blank" rel="noopener noreferrer" className="w-11 h-11 rounded-xl bg-slate-100 hover:bg-gradient-to-br hover:from-brand hover:to-sky flex items-center justify-center transition-all hover:scale-110 hover:text-white text-brand">
...
const inputCls =
  'w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-brand placeholder-slate-400 focus:outline-none focus:border-sky focus:bg-white focus:ring-2 focus:ring-sky/20 transition-all';

function Field({ label, required, children }: { label: string; required?: boolean; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="block text-sm font-semibold text-brand mb-2">
